...
function formatAlertForTelegram(a) {
  ...
  const line = a?.lines?.sharp_entry != null ? (a.lines.sharp_entry >= 0 ? `+${a.lines.sharp_entry}` : `${a.lines.sharp_entry}`) : "-";
...
app.get("/health", guard(async (req, res) => {
  const env = {
    ...
    LEAN_THRESHOLD: Number(process.env.LEAN_THRESHOLD || 0.010),
    STRONG_THRESHOLD: Number(process.env.STRONG_THRESHOLD || 0.020),
...
app.get("/api/scan/:sport", guard(async (req, res) => {
  ...
  if (sport === "nfl") {
    ...
    if (BOOL(process.env.ENABLE_MLB_H2H, false)) { plannedJobs.push("MLB H2H"); jobs.push(getMLBH2HNormalized); }
  } else if (sport === "ncaaf") {
    if (BOOL(process.env.ENABLE_NCAAF_H2H, false)) { plannedJobs.push("NCAAF H2H"); jobs.push(getNCAAFH2HNormalized); }
  } else {
...
// Debug: return one normalized snapshot (no analysis)
...
// Debug: run analyzer on a normalized snapshot
