...
// Expected value per $1 risk, given American odds + model win prob
...
// From one game blob, find best price per team and compute hold + de-vig
  ...
  for (const bm of game.bookmakers ?? []) {
    for (const m of bm.markets ?? []) {
      ...
      for (const o of m.outcomes ?? []) {
        ...
        if (!cur || ip < cur.ip) {
          best.set(team, { book: bm.title, price, ip });
        }
      }
    }
  }
