...
// Env-driven; no external config import. Exports analyzeMarket(snapshot, opts?)
...
function analyzeWithEV(all, target, nonTarget, meta, { bypassDedupe }) {
  ...
  if (tier === "pass") {
    diag(() => console.log(`diag[EV] ${meta.away} @ ${meta.home} | best ${(ev*100).toFixed(2)}% < gates`));
    return null;
  }
  ...
  return finalizeAlert({
    ...meta,
    source: "ev",
    score: Math.round(ev * 10000) / 100,
    tier, side: best.side, team,
    entryLine: best.price, priceBook: best.book, evPct: ev,
    signals: [
...
function normalizeOffers(snap) {
  ...
  for (const o of snap?.offers || []) {
    ...
    seen.add(book); out.push({ book, home, away });
  }
  return out;
}
